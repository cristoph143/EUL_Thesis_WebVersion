...
export class ListOfTeacherComponent implements OnInit, AfterViewInit {
  ...
  ListOfTeacherCopy: ListOfTeacher[] = [];
  ...
  getAllSpecificRole() {
    this.accService.getAllSpecificRoles('teacher').subscribe((data: any) => {
      for (let i = 0; i < data[0].length; i++) {
        if (this.ListOfTeacher.some((item) => item.SchoolID === data[0][i].school_id)) {
          continue;
        }
  ...
  editRow(element: any) {
    let check = false;
    console.log(element)
    console.log(this.ListOfTeacherCopy[0].SchoolID)
    let index = this.ListOfTeacherCopy.findIndex((item: any) => item.id === element.id);
    console.log(index)
    // use index to copy the row in this.ListOfTeacherCopy
    let arr = this.ListOfTeacherCopy[index];
    console.log(arr)
    // check if this.ListOfTeacher is equal to the index of this.ListOfTeacherCopy
    // if it is equal then return true
    // if it is not equal then return false
    if (this.ListOfTeacherCopy.some((item: any) => item.SchoolID === element.SchoolID.trim() &&
    ...
    // use id to get the index of the row in this.ListOfTeacherCopy
    let errCheck = this.errorChecking(element, check, arr);
    console.log(errCheck)
    if (!errCheck) {
      const old_school_id = this.ListOfTeacherCopy[index].SchoolID;
  ...
  private errorChecking(element: any, check: boolean, arr: any) {
    ...
    let SchoolIDCheck = this.ListOfTeacherCopy.some((item: any) => item.SchoolID === element.SchoolID.trim());
    let userNameCheck = this.ListOfTeacherCopy.some((item: any) => item.FirstName === element.FirstName.trim() && item.LastName === element.LastName.trim() && item.SchoolID !== element.SchoolID.trim())
    let emailCheck = this.ListOfTeacherCopy.some((item: any) => item.Email === element.Email.trim());
    let emailAndSchoolIDCheck = this.ListOfTeacherCopy.some((item: any) => item.Email === element.Email.trim() && item.SchoolID !== element.SchoolID.trim())
    ...
    for (let i = 0; i < Object.keys(element).length; i++) {
      ...
      if (Object.keys(element)[i] === 'SchoolID') {
        // get the index of the current SchoolID in this.ListOfTeacherCopy using first_name and last_name as index
  ...
  private schoolIDCheck(SchoolIDCheck: boolean, element: any, idCheck: any, arr: any) {
    if (SchoolIDCheck) {
      // ignore row when filtering this.ListOfTeacherCopy if element.FirstName === item.FirstName && element.LastName === item.LastName
      let copy = this.ListOfTeacherCopy.filter((item: any) => item.FirstName !== element.FirstName.trim() || item.LastName !== element.LastName.trim());
